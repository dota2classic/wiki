  ...
  function updateAllLines() {
    if (parentItems.value.length > 0) {
      parentItems.value.forEach((parentItem, index) => {
        drawItemLine(`ci-${props.currentItem.id}`, `${parentItem.id}-${index}`, false);
      });
    }

    if (childItems.value.length > 0) {
      childItems.value.forEach((childItem, index) => {
        drawItemLine(`ci-${props.currentItem.id}`, `${childItem.id}-${index}`, true);
      });
    }
  }
...
<template>
    <div class="item-tree" :id="props.tree_id">
      <TheTag color="dark">{{t('general.itemtree.name')}}</TheTag>
      <transition name="fade" mode="out-in">
        <span v-if="!currentItem.children && !currentItem.parents" class="no-recipe-holder">
      ...
      <div class="item-list-outer children">
        <transition name="fade" mode="out-in">
            <div class="item-list" v-if="childItems.length">
            <transition appear name="squash">
              <span :key="`${currentItem.id}-children-1__item-list-animation-helper`" class="item-list-animation-helper"></span>
            </transition>
            <RouterLink
              v-for="(child, index) in childItems"
              :id="`rel-${child.id}-${index}`"
              :to="{ name: 'item.show', params: { category: props.category, id: child.id } }"
              class="item"
            >
              <img :src="`/images/${child.image}`" :alt="child.name" :title="child.name" />
              <div class="line-container">
                <svg class="item-line" :id="`line-${child.id}-${index}`" xmlns="http://www.w3.org/2000/svg">
                  <line x1="0" y1="0" x2="0" y2="0" stroke="white" stroke-width="2" />
                </svg>
              </div>
            </RouterLink>
            <transition appear name="squash">
              <span :key="`${currentItem.id}-children-2__item-list-animation-helper`" class="item-list-animation-helper`"></span>
            </transition>
          </div>
        </transition>
      </div>
      ...
      <transition appear @enter="animateLineUpdates">
        <span :key="`${currentItem.id}-event-listener`" style="display: none;"></span>
      </transition>
        <div class="item current-item" :id="`ci-${currentItem.id}`">
          <img :src="`/images/${currentItem.image}`" :alt="currentItem.name" :title="currentItem.name" />
        </div>
  
      <div class="item-list-outer parents">
        <transition name="fade" mode="out-in">
          <div class="item-list" v-if="parentItems.length">
            <transition appear name="squash">
              <span :key="`${currentItem.id}-parents-1__item-list-animation-helper`" class="item-list-animation-helper"></span>
            </transition>
            <RouterLink
              v-for="(parent, index) in parentItems"
              :id="`rel-${parent.id}-${index}`"
              :to="{ name: 'item.show', params: { category: props.category, id: parent.id } }"
              class="item"
            >
              <img :src="`/images/${parent.image}`" :alt="parent.name" :title="parent.name" />
              <div class="line-container">
                <svg class="item-line" :id="`line-${parent.id}-${index}`" xmlns="http://www.w3.org/2000/svg">
                  <line x1="0" y1="0" x2="0" y2="0" stroke="white" stroke-width="2" />
                </svg>
              </div>
            </RouterLink>
            <transition appear name="squash">
              <span :key="`${currentItem.id}-parents-2__item-list-animation-helper`" class="item-list-animation-helper"></span>
            </transition>
          </div>
        </transition>
      </div>
...
<style scoped>
  ...
  .item-tree {
    ...
    border-radius: 3px;
    box-shadow: rgb(13,13,13) 3px 3px 6px 0px inset, rgba(0, 0, 0, 0.1) -3px -3px 6px 1px inset;
